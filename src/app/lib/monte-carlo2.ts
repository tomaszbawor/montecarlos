...
function calculateWorkdays(projectStartDate: Date, projectEndDate: Date) {
        const s = new Date(projectStartDate.getFullYear(), projectStartDate.getMonth(), projectStartDate.getDate())
        const e = new Date(projectEndDate.getFullYear(), projectEndDate.getMonth(), projectEndDate.getDate())
...
export function runMonteCarloSimulationNew(
    ...
    for (let i = 0; i < iterations; i++) {
        ...
        for (const devs of simulationSettings.developers) {
            sickLeavesInDays.push(calculateSickLeaveTimeInDays(devs, simulationSettings.standardDeviationParameter))
            engagementsInPercentage.push(calculateDevEngagementInPercentage(devs, simulationSettings.standardDeviationParameter))
        }

        marketProductivityInPercentage = calculateMarketProductivity(simulationSettings.marketAverageAgileProductivityMaxPercentage,
